...
class JSONMaker:
    ...
    def handleExisted(self, existedFilePath):
        ...
        if (
            existedFilePath is not None
            and Path(existedFilePath).exists()
            and Path(existedFilePath).suffix == ".json"
        ):
            ...
            assert len(existed_imgNames) == len(
                unique_existed_imgIDS
            ), "Error: The image id in map.json is not unique!"
    ...
    def makeJSON(self):
        ...
        for part in ["train", "val", "test"]:
            ...
            with open(
                os.path.join(self.annotationFolder, part + ".json"),
                "w",
                encoding="utf-8",
            ) as f:
        ...
        with open(
            os.path.join(self.annotationFolder, "log.json"), "w", encoding="utf-8"
        ) as f:
    ...
    def showIMG(self):
        ...
        while True:
            ...
            img_ro = cv2.imread(
                str(Path(self.outFolder).joinpath("images", part, imgPath))
            )
            img_no = img_ro.copy()
            ...
            for bboxInfo in selectedBBOX:
                ...
                img_ro = cv2.polylines(
                    img_ro,
                    [np.array(roPoints, np.int32).reshape((-1, 1, 2))],
                    True,
                    (color[categoryIdx, :]).tolist(),
                    2,
                )
            ...
            cv2.putText(
                img_ro,
                "%s,%s,%s,%d,%d,%s with oriented BBOX"
                % (part, imgScenario, imgWeather, imgPespective, imgAltitude, imgDate),
                (5, 10),
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        type=float,
        default=0.7,
        help="Training dataset proportion",
    )
    parser.add_argument(
        "--val_proportion",
        "-V",
    ...
    parser.add_argument(
        ...
        type=str,
        default="../info",
        help="Path to the information folder",
    )
    parser.add_argument(
        "--output",
        "-O",
        type=str,
        default="Pegasus",
        help="Folder for JSON file and log file",
    )
    parser.add_argument(
        "--existedFile", "-M", type=str, default="./existedFile.json", help="images map"
    )
    parser.add_argument(
        "--bboxType", "-B", type=int, default=0, help="OBB: 0, Normal: 1"
    )
